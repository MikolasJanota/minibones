...
#include "picosat_wrapper_ext.hh"
...
int PicosatWrapperExt::run_sat_solver()
{
  maxvid = 0;
  ...
  if (status == 20) {         // Unsatisfiable
    ...
    if (levels.size() < maxvid+1) { levels.resize(maxvid+1, 0); }
    for(unsigned int i=1; i<=maxvid; ++i) {
      model[i] = picosat_deref(i);
      levels[i] = picosat_deref_level(i);
...
/*
void PicosatWrapperExt::get_model(vector<int>& rmodel)
{
...
void PicosatWrapperExt::init() { }

void PicosatWrapperExt::reset()
{
  //model.clear();
  idcls.clear();
}

void PicosatWrapperExt::get_var_levels(IntVector& varlevel)
{
...
bool PicosatWrapperExt::is_dvar(LINT vid)
{
  return picosat_decision_lit(vid) != 0;
}

void PicosatWrapperExt::print_cnf(const char* fname)
{
